package com.dhht.sld.main.updateapp.download.downloadmanager;
...
import com.dhht.sld.main.updateapp.view.UpDateAppDialog;
import com.dhht.sld.utlis.Md5Util;
...
public class UpdateManager implements UpdateDialogListener {
     ...
     * 更新提醒对话框
     */
    private UpDateAppDialog updateRemindDialog;
    ...
    public void startUpdate(Context context, AppUpdate appUpdate) {
        ...
        updateRemindDialog = UpDateAppDialog.newInstance(bundle).addUpdateListener(this);
    ...
    private void downLoadApk() {
        ...
        try {
            ...
            String subFileName = context.getPackageName()+"_"+ appUpdate.getNewVersionCode() + ".apk";
            if (context != null) {
                ...
                if (TextUtils.isEmpty(appUpdate.getSavePath())) {
                    ...
                    request.setDestinationInExternalFilesDir(context, Environment.DIRECTORY_DOWNLOADS, subFileName);
                    deleteApkFile(Objects.requireNonNull(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS + File.separator + subFileName)));
                } else {
                    // 自定义的下载目录,注意这是涉及到android Q的存储权限，建议不要用getExternalStorageDirectory（）
                    request.setDestinationInExternalFilesDir(context, appUpdate.getSavePath(), subFileName);
                    deleteApkFile(Objects.requireNonNull(context.getExternalFilesDir(appUpdate.getSavePath() + File.separator + subFileName)));
                }
    ...
    public void showFail() {
        if (updateRemindDialog != null) {
            updateRemindDialog.downLoadFail();
        }
    }
    ...
    private File checkLocalUpdate(String filePath) {
        try {
            ...
            if (TextUtils.isEmpty(filePath)) {
                apkFile = context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS + File.separator + subFileName);
            } else {
                apkFile = context.getExternalFilesDir(filePath + File.separator + subFileName);
            }
    ...
    public void updateDownLoad() {
        ...
        if (apkFile != null) {
            ...
            if (!appUpdate.getIsSlentMode()) {
                ...
                updateRemindDialog.showProgressBar();
            } else {
                // 静默模式，不显示下载进度
                dismissDialog();
            }
