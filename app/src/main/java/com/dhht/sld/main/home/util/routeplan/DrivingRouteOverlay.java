...
public class DrivingRouteOverlay extends RouteOverlay {
    ...
    private float mWidth = 22;
    private List<LatLng> mLatLngsOfPath;
    private int type;


    public DrivingRouteOverlay(Context context, AMap amap, DrivePath path,
                               LatLonPoint start, LatLonPoint end, List<LatLonPoint> throughPointList,int type) {
        super(context);
        mContext = context;
        mAMap = amap;
        this.drivePath = path;
...
//        mPolylineOptionscolor.add(AMapUtil.convertToLatLng(tmcSection.get(0).getPolyline().get(0)));
        ...
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            distance += segmentTrafficStatus.getDistance(); //距离 米
...
//        mPolylineOptionscolor.setCustomTextureList(bitmapDescriptors);
    ...
    private void initBitmapDescriptor() {
        ...
        defaultRoute = BitmapDescriptorFactory.fromResource(R.drawable.caiwu2);
        /*
